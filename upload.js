...
module.exports = function(tmp){
	...
	app.get('/', function(req, res){
		return $.get(req, function(status, filename, original, identifier) {
			res.sendStatus(status == 'found' ?  200 : 204);
		});
	});

	app.post('/', multipart({uploadDir: tmp}), function(req, res){
		return $.post(req, function(status, filename, original, identifier) {
			res.sendStatus(status=='done' || status=='partly_done' ?  200 : 400);
		});
	});
